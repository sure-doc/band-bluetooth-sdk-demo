...
import { PickerColumn, PickerColumnItem } from './types';
...
export function generateDatePickerColumns(
  ...
  const selectedHour = parseInt(selected[3], 10);
  const selectedMinute = parseInt(selected[4], 10);
...
export function convertDateToStringArray(date: Date | undefined | null, precision: Precision): string[] {
  if (!date) return [];
  const arr = [date.getFullYear().toString()];
  if (precision === 'year') return arr;

  arr.push((date.getMonth() + 1).toString());
  if (precision === 'month') return arr;

  arr.push(date.getDate().toString());
  if (precision === 'day') return arr;

  arr.push(date.getHours().toString());
  if (precision === 'hour') return arr;

  arr.push(date.getMinutes().toString());
  if (precision === 'minute') return arr;

  arr.push(date.getSeconds().toString());
  return arr;
}

export function convertStringArrayToDate(value: (string | null | undefined)[]): Date {
